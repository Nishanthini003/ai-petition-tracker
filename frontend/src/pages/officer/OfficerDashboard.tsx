...
const OfficerDashboard = () => {
  ...
  const [repetitivePetitions, setRepetitivePetitions] = useState<Petition[]>([]);
  ...
  useEffect(() => {
    if (user?.department && petitions.length > 0) {
      ...
      const repetitive = new Set<Petition>();
      deptPetitions.forEach(petition => {
        const similar = findSimilarPetitions(petition, deptPetitions);
        if (similar.length > 0) {
          repetitive.add(petition);
          similar.forEach(p => repetitive.add(p));
        }
      });
      setRepetitivePetitions(Array.from(repetitive));
    }
              ...
              <div className="flex justify-between items-start">
                <div>
                  <div className="flex items-center space-x-2">
                    ...
                    {urgentPetitions.some(p => p._id === petition._id) && (
                      <Tag icon={<ExclamationCircleOutlined />} color="red">Urgent</Tag>
                    )}
                    {repetitivePetitions.some(p => p._id === petition._id) && (
                      <Tag icon={<HistoryOutlined />} color="purple">Repetitive</Tag>
                    )}
                  </div>
  ...
  const renderNotifications = () => (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        <Tag color="blue">{urgentPetitions.length + repetitivePetitions.length} alerts</Tag>
      </div>
        ...
        {repetitivePetitions.length === 0 ? (
          ...
          <div className="space-y-4">
            {repetitivePetitions.map(petition => {
              const similarPetitions = findSimilarPetitions(petition, repetitivePetitions);
              return (
                <div key={petition._id} className="border-l-4 border-purple-500 pl-4 py-2">
                  <div className="flex justify-between items-center">
                    <div>
                      <h4 className="font-medium">{petition.title}</h4>
                      <p className="text-gray-600 text-sm line-clamp-1">{petition.description}</p>
                      <p className="text-gray-600 text-sm line-clamp-1">{petition.submittedBy}</p>
                      <div className="mt-1">
                        <Tag color={statusColors[petition.status]}>
                          {petition.status.replace('_', ' ')}
                        </Tag>
                  ...
                  {similarPetitions.length > 0 && (
                    <div className="mt-1">
                      <Tag color="purple">
                        Similar to {similarPetitions.length} other petition(s)
                      </Tag>
                    </div>
                  )}
                </div>
            ...
            <div className="flex justify-between items-start">
              <div>
                ...
                <div className="mt-2 flex items-center space-x-2">
                  ...
                  {repetitivePetitions.some(p => p._id === selectedPetition._id) && (
                    <Tag icon={<HistoryOutlined />} color="purple">Repetitive</Tag>
                  )}
                </div>
              </div>
